...
local function apply_effect()
    ...
    elseif mode_id == "ambient" then
        ...
        -- Unified 4-Principle Optical Ambilight:
        -- Principle 1: Chameleon Uniformity (Zero Fake Vignette) — solid white stays 100% pure white (255, stdev 0.0), solid red stays pure red.
        -- Principle 2: Convex n-Dome Light Arc — horizontal integration merges window lights into a continuous 'n' dome, removing 'U' inner notches.
        -- Principle 3: Optical Distance Falloff — specular lights shine near the frame and dissolve to pitch black (0) well before the bezel.
        -- Principle 4: Deep Shadow Bipolar Contrast (gamma=0.52, contrast=1.50, brightness=-0.04) — midtones drop to deep darkness (23) while true highlights glow.
        local base_scale = is_letterbox and "48:27" or "27:48"
        local avg_params = is_letterbox and "sizeX=13:sizeY=7" or "sizeX=7:sizeY=13"
        vf_str = string.format(
            "lavfi=[split[fg][bg]; [bg]scale=%s:flags=area,avgblur=%s,format=gbrp,gblur=sigma=4:steps=2,format=yuv420p,tmix=frames=3:weights='1 2 4',scale=%d:%d:flags=bicubic,gradfun=strength=5.0:radius=32,eq=gamma=0.52:contrast=1.50:brightness=-0.04:saturation=1.22[bg_glow]; [bg_glow][fg]overlay=(W-w)/2:(H-h)/2:eof_action=pass:repeatlast=0,setsar=1]",
