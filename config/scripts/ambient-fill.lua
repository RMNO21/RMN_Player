...
local function apply_effect()
    ...
    elseif mode_id == "ambient" then
        ...
        -- 2. Power Gamma Curve (gamma=0.78): Shadows and dark regions become significantly darker/deeper,
        ...
        -- 5. Wide-Radius Debanding: gradfun with restored strength=5.0 and max radius=32 for broad gradient smoothing.
        local base_scale = is_letterbox and "32:18" or "18:32"
        vf_str = string.format(
            "lavfi=[split[fg][bg]; [bg]scale=%s:flags=area,format=gbrp,gblur=sigma=4:steps=2,format=yuv420p,tmix=frames=3:weights='1 2 4',scale=%d:%d:flags=bicubic,gradfun=strength=5.0:radius=32,eq=gamma=0.78:contrast=1.15:saturation=1.20[bg_glow]; [bg_glow][fg]overlay=(W-w)/2:(H-h)/2:eof_action=pass:repeatlast=0,setsar=1]",
